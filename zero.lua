...
-- Config System
local CONFIG_KEY = "ZERO_HUB_CONFIG_V1"
...
function startHitCircle()
    ...
    Cebo.Circle.Color = Color3.fromRGB(200, 120, 220)
...
-- Purple Moon
local purpleMoonCC       = nil
local purpleMoonBloom    = nil
local purpleMoonConn     = nil
local purpleMoonSky      = nil
local purpleMoonFOV      = nil
local purpleMoonBall     = nil

function enablePurpleMoon()
    if purpleMoonCC then return end
    purpleMoonCC = Instance.new("ColorCorrectionEffect")
    purpleMoonCC.TintColor = Color3.fromRGB(200, 120, 220)
    purpleMoonCC.Saturation = 0.6
    purpleMoonCC.Contrast   = 0.15
    purpleMoonCC.Brightness = 0.05
    purpleMoonCC.Parent = Lighting

    purpleMoonBloom = Instance.new("BloomEffect")
    purpleMoonBloom.Intensity  = 1.2
    purpleMoonBloom.Size       = 20
    purpleMoonBloom.Threshold  = 0.7
    purpleMoonBloom.Parent = Lighting

    Lighting.ExposureCompensation = 0.2
    Lighting.Ambient = Color3.fromRGB(60, 25, 60)
    Lighting.OutdoorAmbient = Color3.fromRGB(80, 35, 80)

    purpleMoonBall = Instance.new("Part")
    purpleMoonBall.Name         = "PurpleMoon"
    purpleMoonBall.Anchored     = true
    purpleMoonBall.CanCollide   = false
    purpleMoonBall.CastShadow   = false
    purpleMoonBall.Massless     = true
    purpleMoonBall.Material     = Enum.Material.Neon
    purpleMoonBall.Shape        = Enum.PartType.Ball
    purpleMoonBall.Color        = Color3.fromRGB(200, 100, 220)
    purpleMoonBall.Size         = Vector3.new(60, 60, 60)
    purpleMoonBall.Position     = Vector3.new(0, 800, -500)
    purpleMoonBall.Parent       = workspace

    purpleMoonFOV = Instance.new("ImageLabel")
    purpleMoonFOV.Name               = "PurpleMoonFOV"
    purpleMoonFOV.Size               = UDim2.new(1, 0, 1, 0)
    purpleMoonFOV.Position           = UDim2.new(0, 0, 0, 0)
    purpleMoonFOV.BackgroundTransparency = 1
    purpleMoonFOV.Image              = "rbxassetid://6923796549"
    purpleMoonFOV.ImageColor3        = Color3.fromRGB(200, 120, 220)
    purpleMoonFOV.ImageTransparency  = 0.55
    purpleMoonFOV.ZIndex             = 1
    purpleMoonFOV.Parent             = ScreenGui

    purpleMoonConn = RunService.Heartbeat:Connect(function()
        if not Enabled.Vibrance then return end
        local t = tick()
        local pulse = math.sin(t * 0.9) * 0.08
        if purpleMoonCC then
            purpleMoonCC.Saturation = 0.55 + pulse
            purpleMoonCC.Brightness = 0.04 + pulse * 0.3
        end
        if purpleMoonBloom then
            purpleMoonBloom.Intensity = 1.1 + math.sin(t * 1.3) * 0.3
        end
        if purpleMoonFOV then
            purpleMoonFOV.ImageTransparency = 0.52 + math.sin(t * 0.7) * 0.06
        end
        if purpleMoonBall then
            purpleMoonBall.Color = Color3.fromRGB(
                math.floor(200 + math.sin(t * 0.5) * 40),
                math.floor(100 + math.sin(t * 0.8) * 30),
                math.floor(220 + math.sin(t * 0.6) * 20)
            )
        end
    end)
end
...
function disablePurpleMoon()
    if purpleMoonConn  then purpleMoonConn:Disconnect()    purpleMoonConn  = nil end
    if purpleMoonBloom then purpleMoonBloom:Destroy()       purpleMoonBloom = nil end
    if purpleMoonCC    then purpleMoonCC:Destroy()          purpleMoonCC    = nil end
    if purpleMoonBall  then purpleMoonBall:Destroy()        purpleMoonBall  = nil end
    if purpleMoonFOV   then purpleMoonFOV:Destroy()         purpleMoonFOV   = nil end
...
-- Colors
local PURPLE       = Color3.fromRGB(200, 120, 220)
local PURPLE_LIGHT = Color3.fromRGB(220, 150, 240)
local PURPLE_DARK  = Color3.fromRGB(150, 80, 180)
local BG_DARK    = Color3.fromRGB(18, 8, 18)
local SECTION_BG = Color3.fromRGB(14, 6, 14)
...
local PanelFrame = Create("Frame", {
    ...
    Create("UIStroke", {Color = PURPLE, Thickness = 3, ApplyStrokeMode = Enum.ApplyStrokeMode.Border})
})
...
Create("Frame", {
    ...
    Create("UIGradient", {
        Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0, Color3.fromRGB(28, 10, 28)),
            ColorSequenceKeypoint.new(0.4, Color3.fromRGB(35, 13, 35)),
            ColorSequenceKeypoint.new(0.8, Color3.fromRGB(45, 18, 45)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(30, 11, 30))
        },
        Rotation = 135
    })
})
...
for i = 1, 18 do
    ...
    local p = Create("Frame", {
        BackgroundColor3 = math.random(1,2)==1 and PURPLE or PURPLE_LIGHT,
        BorderSizePixel = 0,
...
chGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.5,  Color3.fromRGB(150, 70,  180)),
    ColorSequenceKeypoint.new(1,    Color3.fromRGB(200, 120, 220)),
})
task.spawn(function()
    local r = 0
...
chBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(chCircle, TweenInfo.new(0.2), {
        BackgroundColor3 = uiVisible and Color3.fromRGB(20, 8, 20) or Color3.fromRGB(50, 20, 50)
    }):Play()
end)
...
local CloseBtn = Create("TextButton", {
    ...
    Create("UICorner", {CornerRadius=UDim.new(1,0)}),
    Create("UIStroke", {Color=PURPLE_LIGHT, Thickness=2})
})
...
local function CreateSection(parent, title, order)
    local sec = Create("Frame", {
        ...
        Create("UIStroke", {Color=PURPLE_DARK, Thickness=1.5, Transparency=0.4})
    })
    Create("TextLabel", {
        BackgroundTransparency = 1,
...
local function CreateToggle(parent, labelText, enabledKey, callback, order)
    local row = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(22, 9, 22),
        ...
        Create("UIStroke", {Color=PURPLE_DARK, Thickness=1.5})
    })
    Create("TextLabel", {
        BackgroundTransparency = 1,
    ...
    local toggleBg = Create("Frame", {
        ...
        BackgroundColor3 = isOn and PURPLE or Color3.fromRGB(55, 22, 55),
        ZIndex = 7,
        Parent = row
    }, {Create("UICorner", {CornerRadius=UDim.new(1,0)})})
    ...
    local function setVisual(state, skipCb)
        isOn = state
        TweenService:Create(toggleBg, TweenInfo.new(0.25), {BackgroundColor3 = isOn and PURPLE or Color3.fromRGB(50,15,50)}):Play()
...
local function CreateSlider(parent, labelText, minVal, maxVal, valueKey, callback, order)
    ...
    local knob = Create("Frame", {
        ...
        Create("UICorner", {CornerRadius=UDim.new(1,0)}),
        Create("UIStroke", {Color=PURPLE, Thickness=2})
    })
...
pbar.BackgroundColor3 = Color3.fromRGB(8, 4, 8)
...
pGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   Color3.fromRGB(200, 120, 220)),
    ColorSequenceKeypoint.new(0.25, Color3.fromRGB(120, 50,  150)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(180, 100, 210)),
    ColorSequenceKeypoint.new(0.75, Color3.fromRGB(100, 40,  130)),
    ColorSequenceKeypoint.new(1,   Color3.fromRGB(200, 120, 220)),
})
task.spawn(function()
    local r = 0
...
ProgressPercentLabel.TextColor3 = Color3.fromRGB(220, 150, 240)
...
statRadius.TextColor3 = Color3.fromRGB(200, 120, 220)
...
pTrack.BackgroundColor3 = Color3.fromRGB(25, 12, 25)
pTrack.ZIndex = 11
Instance.new("UICorner", pTrack).CornerRadius = UDim.new(1, 0)
ProgressBarFill = Instance.new("Frame", pTrack)
ProgressBarFill.Size = UDim2.new(0, 0, 1, 0)
ProgressBarFill.BackgroundColor3 = Color3.fromRGB(200, 120, 220)
...
CreateToggle(ScrollFrame, "🌙 Purple Moon", "Vibrance", function(s)
    Enabled.Vibrance = s
...
saveConfigClickBtn.MouseButton1Click:Connect(function()
    saveConfig()
    TweenService:Create(saveConfigBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(10, 4, 10)}):Play()
    task.delay(2, function()
        TweenService:Create(saveConfigBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(22, 9, 22)}):Play()
    end)
end)
...
useConfigClickBtn.MouseButton1Click:Connect(function()
    TweenService:Create(useConfigBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(10, 4, 10)}):Play()
    applyBootEffect()
    task.delay(2, function()
        TweenService:Create(useConfigBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(22, 9, 22)}):Play()
    end)
end)
...
local function CreateFloatingButton(labelText, icon, defaultPos, enabledKey, onToggle, isToggle)
    ...
    local btnFrame = Create("Frame", {
        BackgroundColor3 = Color3.fromRGB(18, 7, 18),
        Position = buttonPos,
        ...
        Create("UICorner", {CornerRadius = UDim.new(0, 12)}),
        Create("UIStroke", {Color = PURPLE, Thickness = 2, ApplyStrokeMode = Enum.ApplyStrokeMode.Border}),
    })
    ...
    local function updateVisual(state)
        ...
        if isToggle then
            ...
            TweenService:Create(btnFrame, TweenInfo.new(0.2), {BackgroundColor3 = state and Color3.fromRGB(50, 20, 50) or Color3.fromRGB(18, 7, 18)}):Play()
