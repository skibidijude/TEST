...
espFolder.Name = "lightCoordESP"

local PINK = Color3.fromRGB(255, 250, 250)
...
local YELLOW_MAIN   = Color3.fromRGB(255, 215,  60)
local YELLOW_BRIGHT = Color3.fromRGB(255, 235, 100)
local YELLOW_GOLD   = Color3.fromRGB(255, 195,  70)
local GOLD_STROKE   = Color3.fromRGB(255, 220,   0)

local theme = {
	bg         = Color3.fromRGB(18, 16, 10),
	white      = Color3.fromRGB(255, 248, 220),
	subtext    = Color3.fromRGB(210, 190, 120),
	toggleOff  = Color3.fromRGB(55, 50, 30),
	toggleOn   = YELLOW_MAIN,
	knob       = Color3.fromRGB(255, 255, 230),
	btnIdle    = Color3.fromRGB(65, 60, 40),
	btnActive  = YELLOW_GOLD,
	carpetBtn  = Color3.fromRGB(245, 185,  50),
	bindIdle   = Color3.fromRGB(75, 65, 35),
	bindListen = Color3.fromRGB(255, 235, 100),
	ibOff      = Color3.fromRGB(150, 125, 60),
	ibOn       = Color3.fromRGB(255, 220,  60),
}

local gui = Instance.new("ScreenGui", CoreGui)
gui.Name = "LightHub"; gui.ResetOnSpawn = false

local frame = Instance.new("Frame", gui)
frame.Name = "MainFrame"
frame.Size = UDim2.new(0, 200, 0, 380)
frame.Position = UDim2.new(1, -210, 0.5, -190)
...
stroke.Thickness = 2.2; stroke.Color = GOLD_STROKE
...
gradient.Color = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(0.75, Color3.fromRGB(255, 225,  80)),
...
title.Size = UDim2.new(1,0,0,24); title.Position = UDim2.new(0,0,0,5)
title.BackgroundTransparency = 1; title.Text = "LIGHT HUB"
title.TextColor3 = YELLOW_BRIGHT; title.TextSize = 14; title.Font = Enum.Font.GothamBold

local divider = Instance.new("Frame", frame)
divider.Size = UDim2.new(0.85,0,0,1); divider.Position = UDim2.new(0.075,0,0,29)
divider.BackgroundColor3 = YELLOW_GOLD; divider.BorderSizePixel = 0

local subtitle = Instance.new("TextLabel", frame)
subtitle.Size = UDim2.new(1,0,0,12); subtitle.Position = UDim2.new(0,0,0,32)
subtitle.BackgroundTransparency = 1; subtitle.Text = "Beta v1.3.0"
subtitle.TextColor3 = theme.subtext; subtitle.TextSize = 9; subtitle.Font = Enum.Font.GothamMedium

local topRow = Instance.new("Frame", frame)
topRow.Size = UDim2.new(1,-16,0,26); topRow.Position = UDim2.new(0,8,0,48)
...
leftBtn.TextColor3 = theme.white; leftBtn.TextSize = 10; leftBtn.Font = Enum.Font.GothamBold
leftBtn.AutoButtonColor = false; leftBtn.BorderSizePixel = 0
Instance.new("UICorner", leftBtn).CornerRadius = UDim.new(0,5)
local s1 = Instance.new("UIStroke", leftBtn); s1.Thickness = 1.2; s1.Color = GOLD_STROKE
...
rightBtn.TextColor3 = theme.white; rightBtn.TextSize = 10; rightBtn.Font = Enum.Font.GothamBold
rightBtn.AutoButtonColor = false; rightBtn.BorderSizePixel = 0
Instance.new("UICorner", rightBtn).CornerRadius = UDim.new(0,5)
local s2 = Instance.new("UIStroke", rightBtn); s2.Thickness = 1.2; s2.Color = GOLD_STROKE
...
Instance.new("UICorner", carpetScamBtn).CornerRadius = UDim.new(0,5)
local sc = Instance.new("UIStroke", carpetScamBtn); sc.Thickness = 1.4; sc.Color = YELLOW_BRIGHT

local page = Instance.new("Frame", frame)
page.Size = UDim2.new(1,-16,1,-110); page.Position = UDim2.new(0,8,0,82)
...
footer.TextColor3 = Color3.fromRGB(85,85,85); footer.TextSize = 9; footer.Font = Enum.Font.GothamMedium
...
local function setToggleVisual(sw, knob, on)
	...
	TweenService:Create(knob, TweenInfo.new(0.22, Enum.EasingStyle.Quart), {Position = on and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,3,0.5,-7)}):Play()
end
...
local function stopListening()
	...
	b.label.TextColor3 = YELLOW_BRIGHT
end
...
local function makeToggleRow(parent, labelText, bindKey, yPos)
	...
	row.Size = UDim2.new(1,0,0,28); row.Position = UDim2.new(0,0,0,yPos)
	row.BackgroundTransparency = 1
	local lbl = Instance.new("TextLabel", row)
	lbl.Size = UDim2.new(1,-90,1,0); lbl.BackgroundTransparency = 1
	...
	bindFrame.Size = UDim2.new(0,26,0,18)
	bindFrame.Position = UDim2.new(1,-64,0.5,-9)
	...
	bStr.Thickness = 1.1; bStr.Color = YELLOW_GOLD
	...
	bindLbl.TextColor3 = YELLOW_BRIGHT
	bindLbl.TextSize = 9; bindLbl.Font = Enum.Font.GothamBold
	bindFrame.MouseButton1Click:Connect(function()
		...
		bindLbl.Text = "..."; bindLbl.TextColor3 = YELLOW_BRIGHT
	end)
	allBindLabels[bindKey] = bindLbl
	local switchBg = Instance.new("TextButton", row)
	switchBg.Size = UDim2.new(0,34,0,18); switchBg.Position = UDim2.new(1,-34,0.5,-9)
...
local stealSwitch,        stealKnob        = makeToggleRow(page, "Steal Boost",      "STEAL",        6)
local jumpSwitch,         jumpKnob         = makeToggleRow(page, "Infinite Jump",    "JUMP",         6 + ySpacing)
local instaGrabSwitch,    instaGrabKnob    = makeToggleRow(page, "Insta Grab",       "INSTAGRAB",    6 + ySpacing*2)
local xraySwitch,         xrayKnob         = makeToggleRow(page, "XRay + Optimizer", "XRAY",         6 + ySpacing*3)
local autoWalkSwitch,     autoWalkKnob     = makeToggleRow(page, "Auto Walk",        "AUTOWALK",     6 + ySpacing*4)
local targetPlayerSwitch, targetPlayerKnob = makeToggleRow(page, "Target Player",    "TARGETPLAYER", 6 + ySpacing*5)

local delayRow = Instance.new("Frame", page)
delayRow.Size = UDim2.new(1,0,0,20)
delayRow.Position = UDim2.new(0,0,0,6 + ySpacing*6 + 4)
delayRow.BackgroundTransparency = 1

local delayLbl = Instance.new("TextLabel", delayRow)
delayLbl.Size = UDim2.new(0,40,1,0)
delayLbl.Position = UDim2.new(0,0,0,0)
delayLbl.BackgroundTransparency = 1
delayLbl.Text = "Delay"
delayLbl.TextColor3 = theme.white
...
delayBox.Size = UDim2.new(0,44,1,0)
delayBox.Position = UDim2.new(0,44,0,0)
delayBox.Text = tostring(blockDelay)
delayBox.TextColor3 = YELLOW_BRIGHT
delayBox.Font = Enum.Font.GothamMedium
delayBox.TextSize = 10
delayBox.BackgroundColor3 = Color3.fromRGB(40,35,20)
delayBox.PlaceholderText = "0.1-5"
delayBox.PlaceholderColor3 = Color3.fromRGB(180,160,100)
delayBox.BorderSizePixel = 0
Instance.new("UICorner", delayBox).CornerRadius = UDim.new(0,4)
local dbStroke = Instance.new("UIStroke", delayBox); dbStroke.Thickness = 1.1; dbStroke.Color = YELLOW_GOLD

local setDelayBtn = Instance.new("TextButton", delayRow)
setDelayBtn.Size = UDim2.new(0,28,1,0)
setDelayBtn.Position = UDim2.new(0,92,0,0)
setDelayBtn.BackgroundColor3 = Color3.fromRGB(120,100,40)
setDelayBtn.Text = "SET"; setDelayBtn.TextColor3 = theme.white
setDelayBtn.TextSize = 9; setDelayBtn.Font = Enum.Font.GothamBold
setDelayBtn.AutoButtonColor = false; setDelayBtn.BorderSizePixel = 0
Instance.new("UICorner", setDelayBtn).CornerRadius = UDim.new(0,4)
...
delayBox:GetPropertyChangedSignal("Text"):Connect(function()
	...
	if num and num >= minDelay and num <= maxDelay then
		dbStroke.Color = YELLOW_GOLD
	else
		dbStroke.Color = Color3.fromRGB(200,60,60)
	end
end)

local ibRowY = 6 + ySpacing*6 + 28

local instaBlockBtn = Instance.new("TextButton", page)
instaBlockBtn.Size = UDim2.new(1,-40,0,24)
...
instaBlockBtn.TextColor3 = theme.white; instaBlockBtn.TextSize = 10
instaBlockBtn.Font = Enum.Font.GothamBold
instaBlockBtn.AutoButtonColor = false; instaBlockBtn.BorderSizePixel = 0
Instance.new("UICorner", instaBlockBtn).CornerRadius = UDim.new(0,5)

local ibBindFrame = Instance.new("TextButton", page)
ibBindFrame.Size = UDim2.new(0,34,0,24)
ibBindFrame.Position = UDim2.new(1,-34,0,ibRowY)
...
Instance.new("UICorner", ibBindFrame).CornerRadius = UDim.new(0,4)
local ibBindStr = Instance.new("UIStroke", ibBindFrame)
ibBindStr.Thickness = 1.1; ibBindStr.Color = YELLOW_GOLD
...
ibBindLbl.TextColor3 = YELLOW_BRIGHT
ibBindLbl.TextSize = 9; ibBindLbl.Font = Enum.Font.GothamBold
...
ibBindFrame.MouseButton1Click:Connect(function()
	...
	ibBindLbl.Text = "..."; ibBindLbl.TextColor3 = YELLOW_BRIGHT
end)
...
task.spawn(function()
	...
	TweenService:Create(frame, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
		Position = UDim2.new(1, -210, 0.5, -190)
	}):Play()
end)
