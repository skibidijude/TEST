--// WindUI v1.6.63 - Protected & Indetectable Version
--// Author: Footagesus | Modified for stealth

local function _G_obf(key)
    return _G[key]
end

local function get_safe_service(name)
    return _G_obf("game"):GetService(name)
end

local Players = get_safe_service("Players")
local HttpService = get_safe_service("HttpService")
local UserInputService = get_safe_service("UserInputService")
local RunService = get_safe_service("RunService")

local LocalPlayer = Players.LocalPlayer
local CoreGui = _G_obf("game"):GetService("CoreGui")

--// Detect UI parent safely
local function get_ui_parent()
    if _G_obf("gethui") then
        return _G_obf("gethui")()
    elseif _G_obf("syn") and _G_obf("syn").protector_enabled then
        return CoreGui
    else
...
local function safe_protect(gui)
    if _G_obf("syn") and _G_obf("syn").protect_gui then
        _G_obf("syn").protect_gui(gui)
    elseif _G_obf("protectgui") then
        _G_obf("protectgui")(gui)
    end
...
local env = setmetatable({
    game = _G_obf("game"),
    workspace = _G_obf("workspace"),
    print = function() end,
    warn = function() end,
    require = nil,
    loadstring = _G_obf("loadstring"),
    setclipboard = _G_obf("setclipboard") or _G_obf("toclipboard"),
    writefile = _G_obf("writefile"),
    readfile = _G_obf("readfile"),
    isfile = _G_obf("isfile"),
    delfile = _G_obf("delfile"),
    makefolder = _G_obf("makefolder"),
    isfolder = _G_obf("isfolder"),
    getcustomasset = _G_obf("getcustomasset"),
    cloneref = _G_obf("cloneref") or _G_obf("clonereference"),
    identifyexecutor = _G_obf("identifyexecutor"),
    request = _G_obf("request") or (_G_obf("syn") and _G_obf("syn").request) or _G_obf("http_request"),
    http_request = _G_obf("http_request") or _G_obf("request"),
    gethwid = _G_obf("gethwid"),
    protectgui = _G_obf("protectgui"),
    syn = _G_obf("syn"),
...
Active=false,
Visible=false,
Parent=ae.Parent or(ad and d.UIElements and d.UIElements.Main and d.UIElements.Main.Main)
},{
ab("UICorner",{
...
ac("UIListLayout",{
Padding=UDim.new(0,ak.PaddingX),
FillDirection="Horizontal",
VerticalAlignment="Center"
}),
al,
...
ad.NewRoundFrame(99,"Squircle",{
Size=UDim2.new(0,aj.Window.NewElements and(ak.ThumbSize*2)or(ak.ThumbSize+4)or(ak.ThumbSize+2)),
Position=UDim2.new(1,0,0.5,0),
AnchorPoint=Vector2.new(0.5,0.5),
ThemeTag={
...
af(ak.UIElements.SliderIcon.Frame.Thumb,.2,{ImageTransparency=0,Size=UDim2.new(0,aj.Window.NewElements and(ak.ThumbSize*2)or(ak.ThumbSize))+8,0,ak.ThumbSize+8)},Enum.EasingStyle.Quint,Enum.EasingDirection.InOut):Play()
end
...
ak("Frame",{
BackgroundTransparency=1,
Size=UDim2.new(1,-7,0,7)+(ao.MenuPadding*3))or(ao.MenuPadding*2)
},
Visible=an.SearchBarEnabled,
},{
ak("UICorner",{
...
local ao=af.New(am.Code,am.Title,al.Parent,function()
if an then
local an=am.Title or"code"
